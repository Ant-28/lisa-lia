    ...
    */
      ...
      TacticSubproofWithResult[Biased]{
        int match {
          ...
          case x + y => {
            ...
            val eqs = SSet(have(proof).bot, have(lp).bot,  have(rp).bot).map(x => x.firstElemR)
            have(eqs |- x + y === x + y) by Restate // should be rightsubsteq
            thenHave(eqs |- x + y === lru + rru) by RightSubstEq.withParameters(
              Seq((x, lru), (y, rru)),
              (Seq(a, b), x + y === a + b)
            )
            thenHave(eqs |- x + y === sumu) by RightSubstEq.withParameters(
              Seq((lru + rru, sumu)),
              (Seq(a), x + y === a)
            )
            have(x + y === sumu) by Tautology.from(lastStep, have(lp), have(lp), have(proof))
            res = sum
          }
        }
      }(res)
    ...
    def evalPlus(using lib: library.type, proof: lib.Proof)(ex: Biased, ey: Biased): (Biased, proof.ProofTacticJudgement) = {
      ...
      case (RB(`-`(`1`)), RB(xs)) => {
        ...
        case (`1` + xss) => {
          have((`1` ∈ R, `-`(`1`) ∈ R, xss ∈ R) |- (`-`(`1`) + ((`1`) + xss)) === xss) by Tautology.from(mone_one_xs_xs of (x := xss))
          res = RB(xss)
        }
